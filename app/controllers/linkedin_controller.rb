class LinkedinController < ApplicationController
  ...
  def oauth_account
    client = LinkedIn::Client.new('77eurldofdom97', 'ek6O7Kz4KeFscJgk', @@config)
    pin = params[:oauth_verifier]
    if pin
      atoken, asecret = client.authorize_from_request(session[:rtoken], session[:rsecret], pin)
      LinkedinOauthSetting.create!(:asecret => asecret, :atoken => atoken, :user_id => current_kayttaja.id)
      linkedin_user
    else
      client.authorize_from_access(linkedin_user.atoken, linkedin_user.asecret)
    end
    user = client.profile(:fields => ["first-name", "last-name", "maiden-name", "formatted-name", :headline, :location, :industry, :summary, :specialties, "picture-url", "public-profile-url"])
    @nimi = user.first_name
    @sukunimi = user.last_name
    @otsikko = user.headline
    @kuva = user.picture_url
    #companies = user.positions.map{|t| t.company}.join ', '
    puts "Koko                                                             : #{user.inspect}"
    puts "#{@nimi}"
    puts "#{@sukunimi}"
    puts "#{@otsikko}"
    puts "#{@kuva}"
  ...
  def remove_oauth
    linkedin_user.destroy!
    linkedin_user
    redirect_to '/'
  end

  def generate_linkedin_oauth_url
    if current_kayttaja.present? && LinkedinOauthSetting.find_by_user_id(current_kayttaja.id).nil?
      client = LinkedIn::Client.new('77eurldofdom97', 'ek6O7Kz4KeFscJgk') #, @@config)
